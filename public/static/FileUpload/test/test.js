...
$(function () {
        ...
        var param = {files: [
            ...
            fu = $('#fileupload').fileupload({
                ...
                send: function () {
                    ...
                    loadIndex += 1;
                    strictEqual(sendIndex, loadIndex, 'uploads in order');
                },
                fail: function (e, data) {
                    strictEqual(data.errorThrown, 'abort', 'uploads aborted');
                },
                stop: function () {
                    start();
                }
                ...
                fu = $('#fileupload').fileupload({
                    ...
                    always: function (e, data) {
                        strictEqual(
                            data.contentType,
                            param.files[0].type,
                            'non-multipart uploads sets file type as contentType'
                        );
                        strictEqual(
                            data.headers['Content-Disposition'],
                            'attachment; filename="' + param.files[0].name + '"',
                            'non-multipart uploads sets Content-Disposition header'
                        );
                        start();
                    }
                });
    ...
    test('Buttonbar event listeners', function () {
        ...
        $('#fileupload').fileupload({
            send: function () {
                ok(true, 'Started file uploads via global start button');
            },
            fail: function (e, data) {
                ok(true, 'Canceled file uploads via global cancel button');
                data.context.remove();
            },
            destroy: function () {
    ...
    test('autoUpload', function () {
        ...
        $('#fileupload')
            .fileupload({
                autoUpload: true,
                send: function () {
                    ok(true, 'Started file uploads automatically');
                    return false;
                }
            })
